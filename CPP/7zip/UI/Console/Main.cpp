    ...
    )
{
	...
	if (codecs->Formats.Size() == 0 &&
		...
		options.Command.CommandType == NCommandType::kList || options.Command.IsFromUpdateGroup()))
		throw kNoFormats;
	...
	if (options.Command.CommandType == NCommandType::kInfo) {
		...
		if (options.Method.CompareNoCase(L"CRC") == 0) {
			HRESULT res = CrcBenchCon((FILE *) stdStream, options.NumIterations, options.NumThreads,
		...
		if (isExtractGroupCommand) {
			int l;
			while (fgets(password, 128, stdin) != NULL) {
				...
				l = strlen(password);
				password[l - 1] = 0;
				HRESULT result = DecompressArchives(codecs,
				    formatIndices,
